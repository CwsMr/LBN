...
file = loadmat('post-operative.mat')
data = file['Data']
X = normalize(data)
label = file['Label'][0]
y = LabelBinarizer().fit_transform(label)

xs = tf.placeholder(tf.float32,shape=[None,8])
ys = tf.placeholder(tf.float32,shape=[None,4])
...
l1 = add_layers(xs,8,10,activation_function=tf.nn.relu)
predict = add_layers(l1,10,4,activation_function=tf.nn.softmax)
