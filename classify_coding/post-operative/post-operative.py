...
wb = xlrd.open_workbook('post-operative.xls').sheet_by_name('Sheet1')
label = wb.col_values(-1)
data = [wb.row_values(i)[:-1]for i in range(wb.nrows)]
data = np.asarray(data)
label = LabelEncoder().fit_transform(np.asarray(label))
savemat('post-operative',{'Data':data,'Label':label})
data = normalize(data)
predict = []
