...
file = loadmat('parkinsons.mat')
data = file['Data']
X = normalize(data)
label = file['Label'][0]
y = LabelBinarizer().fit_transform(label)
print(y,X.shape,y.shape)
xs = tf.placeholder(tf.float32,shape=[None,23])
ys = tf.placeholder(tf.float32,shape=[None,7])
...
l1 = add_layers(xs,23,20,activation_function=tf.nn.relu)
predict = add_layers(l1,20,7,activation_function=tf.nn.softmax)
